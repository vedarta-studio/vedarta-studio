    ...
    <section id="contact" className="py-24 bg-[#041a16] relative">
      ...
      {showSocialManifesto && (
        <div className="fixed inset-0 z-[1000] bg-[#041a16]/95 backdrop-blur-2xl overflow-y-auto animate-fade-in">
          <div
            className="min-h-screen flex justify-center md:items-center md:p-6"
            style={{ paddingTop: 'max(env(safe-area-inset-top), 24px)', paddingRight: '16px', paddingBottom: '16px', paddingLeft: '16px' }}
          >
            <div className="relative group max-w-4xl w-full bg-white/[0.02] border border-white/5 p-4 md:p-12 rounded-sm shadow-2xl">
              <div className="text-[#f5f5dc]/80 text-base md:text-lg leading-relaxed italic font-serif space-y-2 pt-2 md:pt-12">
                {siteConfig.socialManifesto.map((line, i) => (
                  <p key={i}>{line}</p>
                ))}
              </div>
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 border-t border-white/5 mt-6 md:mt-8 pt-6 md:pt-8">
                <div className="flex items-center gap-4 md:gap-6">
                  <img src={siteConfig.founder.photo} className="w-14 h-14 md:w-16 md:h-16 rounded-full object-cover border-2 border-[#d4af37]/20" />
                  <div>
                    <h4 className="text-xl md:text-2xl font-logo italic text-[#d4af37]">{siteConfig.founder.name}</h4>
                    <p className="text-[8px] text-[#d4af37]/40 tracking-widest uppercase font-black">Founder of VedArta</p>
                  </div>
                </div>
                <button onClick={() => setShowSocialManifesto(false)} className="self-start md:self-auto text-[9px] font-black uppercase text-[#d4af37] border-b border-[#d4af37]/10 pb-1">Вернуться</button>
              </div>
            </div>
          </div>
        </div>
